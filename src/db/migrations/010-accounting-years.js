...
async function up() {
  logger.info('Running migration: 010-accounting-tables');
  ...
  try {
    ...
    await db.query(`
      CREATE TABLE IF NOT EXISTS accounting_totals (
        ...
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
        PRIMARY KEY (account_number, year, period_number, agreement_number),
        FOREIGN KEY (year, agreement_number) REFERENCES accounting_years(year, agreement_number),
        INDEX idx_period_year (period_number, year, agreement_number)
      ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
    `);
    
    logger.info('Migration 010-accounting-tables completed successfully');
  } catch (error) {
    logger.error('Error running migration 010-accounting-tables:', error.message);
    throw error;
  }
}

async function down() {
  logger.info('Reverting migration: 010-accounting-tables');
  ...
  try {
    ...
    logger.info('Migration 010-accounting-tables reverted successfully');
  } catch (error) {
    logger.error('Error reverting migration 010-accounting-tables:', error.message);
    throw error;
  }
}
